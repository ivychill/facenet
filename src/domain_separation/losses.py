...
def dann_loss(source_samples, target_samples, weight, scope=None):
  ...
  with tf.variable_scope('dann'):
    ...
    grl = flip_gradient(samples, 1)
    grl = tf.reshape(grl, (-1, samples.get_shape().as_list()[1]))

    grl = slim.fully_connected(grl, 1024, activation_fn=tf.nn.relu, scope='fc1')
    grl = slim.fully_connected(grl, 1024, activation_fn=tf.nn.relu, scope='fc2')
