...
class Associative(object):
    def __init__(self, network, args):
        self.args = args
        NUM_LABELS = args.people_per_batch  # len(...)
        ...
        assert args.people_per_batch <= supervise_nrof_classes
    ...
    def get_loss(self, t_sup_images_ph, t_unsup_images_ph, t_sup_labels_ph):
        # Compute embeddings and logits.
        # t_sup_images_ph = tf.placeholder(shape=[None, self.args.image_size, self.args.image_size, 3], dtype=tf.int8)
        # t_unsup_images_ph = tf.placeholder(shape=[None, self.args.image_size, self.args.image_size, 3], dtype=tf.int8)
        # t_sup_labels_ph = tf.placeholder(shape=[None, self.args.embedding_size], dtype=tf.int8)
        t_sup_emb, _, _, _, _ = self.model.image_to_embedding(t_sup_images_ph, self.args.keep_probability, reuse=tf.AUTO_REUSE)
        t_unsup_emb, _, _, _, _ = self.model.image_to_embedding(t_unsup_images_ph, self.args.keep_probability, reuse=tf.AUTO_REUSE)

        t_sup_emb = tf.nn.l2_normalize(t_sup_emb, 1, 1e-10)
        t_unsup_emb = tf.nn.l2_normalize(t_unsup_emb, 1, 1e-10)

        return self.model.add_semisup_loss(t_sup_emb, t_unsup_emb, t_sup_labels_ph, visit_weight=1.0)
    ...
    def sample_data(self, dataset, sampled_class_index):
        ...
        for label, index in enumerate(sampled_class_index):
            ...
            if nrof_images_in_class > self.args.images_per_person:
                sampled_image_paths = random.sample(dataset[index].image_paths, self.args.images_per_person)
            else:
