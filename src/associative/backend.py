...
class SemisupModel(object):
  ...
  def __init__(self, model_func, num_labels, input_shape, keep_probability, test_in=None):
    ...
    # self.test_emb, _, _, _, _ = self.image_to_embedding(self.test_in, keep_probability, is_training=False)
    # self.test_logit = self.embedding_to_logit(self.test_emb, is_training=False)

  def image_to_embedding(self, images, keep_probability, is_training=True, reuse=None):
    """Create a graph, transforming images into embedding vectors."""
#    with tf.variable_scope('net', reuse=is_training):
    return self.model_func(images, keep_probability, phase_train=is_training, reuse=reuse)
...
#          weights_regularizer=slim.l2_regularizer(1e-4))
  ...
  def add_semisup_loss(self, a, b, labels, walker_weight=1.0, visit_weight=1.0):
    ...
    # logger.debug("loss_aba: %s, visit_loss: %s" % (loss_aba, visit_loss))
